    ...
    return (<>
          ...
          <div class="relative h-min flex-grow">
            <Slider class="carousel carousel-center gap-6 w-full">
              {images.map((img, index) => (<Slider.Item index={index} class="carousel-item w-full">
                  <ImageZoom classes="w-full h-full" aspect={ASPECT_RATIO} src={img.url!} alt={img.alternateName} width={WIDTH} height={HEIGHT} 
      ...
      >
        ...
        <div class="col-start-1 col-span-1">
          <ul class={clx("carousel carousel-center", "sm:carousel-vertical", "gap-2", "max-w-full", "overflow-x-auto", "sm:overflow-y-auto")} style={{ maxHeight: "600px" }}>
            {images.map((img, index) => (<li class="carousel-item w-16 h-16">
                <Slider.Dot index={index}>
                  <Image style={{ aspectRatio: "1 / 1" }} class="group-disabled:border-base-400 border rounded object-cover w-full h-full" width={64} height={64} src={img.url!} alt={img.alternateName}/>
                </Slider.Dot>
              </li>))}
          </ul>
        </div>
      ...
      <script type="module" dangerouslySetInnerHTML={{ __html: useScript(ProductImageZoom) }}/>
    </>);
}
