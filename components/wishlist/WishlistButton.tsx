...
const onLoad = (id: string, productID: string) => window.STOREFRONT.WISHLIST.subscribe((sdk) => {
    ...
    button.querySelector("svg")?.setAttribute("fill", inWishlist ? "#bea669" : "none");
});
...
function WishlistButton({ item }: Props) {
    ...
    return (<>
      <button id={id} data-wishlist-button disabled {...addToWishlistEvent} aria-label="Add to wishlist" hx-on:click={useScript(onClick, productID, productGroupID)} class="group/wishlist-button">
        <Icon id="favorite-button" class="[.htmx-request_&]:hidden text-white group-hover/wishlist-button:text-[#bea669] duration-200" fill="currentColor"/>
