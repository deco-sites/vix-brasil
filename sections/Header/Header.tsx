...
import { NAVBAR_HEIGHT_MOBILE, SEARCHBAR_DRAWER_ID, SEARCHBAR_POPUP_ID, SIDEMENU_CONTAINER_ID, SIDEMENU_DRAWER_ID, } from "../../constants.ts";
...
const Desktop = ({ navItems, logo, searchbar, loading }: Props) => (<>
    <div class="flex flex-col gap-4 max-w-full px-6">
      <div class="grid grid-cols-header place-items-center ">
        <div id="vix-brasil-logo" class="self-center place-self-start ease-in duration-200 group-hover/header:invert-0 group-hover/header:brightness-100 m">
        ...
        <div id="vix-brasil__header-icons" class="w-full justify-end self-center flex gap-8 place-self-end items-center group-hover/header:text-black duration-200">
          <div>
            {loading === "lazy"
        ...
        : <Searchbar {...searchbar}/>}
          ...
          <label id="vix-brasil__search-bar--open" for={SEARCHBAR_POPUP_ID} class="flex items-center cursor-pointer group-hover/header:text-black duration-200 z-10" aria-label="search icon button">
...
function Header({ logo = {
    src: "https://vixbrasil.vtexassets.com/assets/vtex/assets-builder/vixbrasil.store/3.0.128/img/header/icon-logo-color___89754752d03a11168a5440e6da2b721b.svg",
    width: 257,
    height: 57,
    alt: "Logo Vix Brasil",
}, ...props }: Props) {
    ...
    // }}
    class="z-50">
      <div class={`group/header backdrop-blur-xs w-full z-40 hover:bg-base-100 hover:backdrop-blur-none ease-in duration-200`} id="vix-brasil-header">
        {/* {alerts.length > 0 && <Alert alerts={alerts} />} */}
        {device === "desktop"
            ? <Desktop logo={logo} {...props}/>
            : <Mobile logo={logo} {...props}/>}
      </div>
      <script type="module" dangerouslySetInnerHTML={{ __html: useScript(HeaderFunctions) }}/>
    </header>);
}
